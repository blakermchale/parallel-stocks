...
if __name__ == '__main__':
    ...
    elasticParams = [0, 1]
    regs = [0, 0.1, 0.3, 0.5, 1, 10, 100, 1000, 2000, 3000, 5000]

    min_mae = float('inf')
    best_reg = 0
    best_elastic_param = 0
    ...
    for elasticParam in elasticParams:
        for reg in regs:
            print("_" * 50)
            print(f"Reg = {reg}")
            print(f"elasticParam = {elasticParam}")

            # create model
            now = time()
            dt = LinearRegression(featuresCol='features', labelCol='Weighted_Price', elasticNetParam=elasticParam, regParam=reg)
            ...
            if (mae < min_mae):
                min_mae = mae
                best_reg = reg
                best_elastic_param = elasticParam
    ...
    print(f"Best elasticParam: {best_elastic_param}")

    # create model based on best hyperparameters
    now = time()
    dt = LinearRegression(featuresCol='features', labelCol='Weighted_Price', elasticNetParam=best_elastic_param, regParam=best_reg)
    dt_model = dt.fit(train_data)
    end = time() - now
    ...
    pred_df.to_csv('../data/predictions/lr_mllib_y_pred.csv', index=False)
    print("Done")
